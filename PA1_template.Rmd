...
```{r}
hist(step_agg, xlab="Daily steps", main="Histogram of total daily steps", ylim=c(0,30))
```
It is also informative to view the data as a bar plot:
```{r}
barplot(step_agg, names=format(as.Date(names(step_agg)),"%d"), ylab="Total steps", xlab="Oct-Nov 2012", cex.names=0.5)
...
head(merged_data)
merged_data[is.na(merged_data$steps),]$steps<-merged_data[is.na(merged_data$steps),]$steps_mean
head(merged_data)
```
...
barplot(merged_step_agg, names=format(as.Date(names(merged_step_agg)),"%d"), ylab="Total steps", xlab="Oct-Nov 2012", cex.names=0.5)
...
```{r}
hist(merged_step_agg, xlab="Daily steps", main="Histogram of total daily steps", ylim=c(0,35))
```
...
```{r}
merged_data$factor<- ifelse(weekdays(as.Date(merged_data$date)) == "Saturday" | weekdays(as.Date(merged_data$date)) == "Sunday", "weekend", "weekday")
head(merged_data)
```
We can then summarise the data by averaging over each interval as before for both weekend and weekday days:
```{r}
dayofweek<-tapply(merged_data$steps, list(merged_data$interval,merged_data$factor),mean)
